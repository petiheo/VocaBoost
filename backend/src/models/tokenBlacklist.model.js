...
class TokenBlacklistModel {
   ...
   */
  static async addToBlacklist(token, tokenType, userId, expiresAt, reason = 'logout') {
    try {
  ...
  static async cleanupExpired() {
    try {
      ...
      const { data: rpcData, error: rpcError } = await supabase.rpc('cleanup_expired_blacklist_tokens');
